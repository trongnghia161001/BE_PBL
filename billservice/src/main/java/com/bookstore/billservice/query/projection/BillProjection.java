...
public class BillProjection {
    ...
    @QueryHandler
    public BillResponseCommonModel getBillByById(GetDetailsBillByIdQuery query) {
        BillResponseCommonModel model = new BillResponseCommonModel();
        Optional<Bill> bill = repository.findById(query.getId());
        if (bill.isPresent()) {
            BeanUtils.copyProperties(bill.get(), model);
        }
        return model;
    }
