package com.bookstore.shoppingcartservice.command.saga;
...
public class ShoppingCartSaga {
    ...
    @Autowired
    private transient KafkaTemplate kafkaTemplate;

    @StartSaga
    @SagaEventHandler(associationProperty = "id")
    private void handle(ShoppingCartCreateEvent event) {
        System.out.println("ShoppingCartCreatedEvent in Saga for ProductId : " + event.getProductId() + " : UserId :  " + event.getUserId());
        try {
            SagaLifecycle.associateWith("id", event.getProductId());
            GetDetailsProductQuery getDetailsBookQuery =
                    new GetDetailsProductQuery(event.getProductId());
            ProductResponseCommonModel productResponseCommonModel = queryGateway.query(getDetailsBookQuery, ResponseTypes.instanceOf(ProductResponseCommonModel.class)).join();
            if (productResponseCommonModel != null) {
                if ((productResponseCommonModel.getNumber() - productResponseCommonModel.getSoldQuantity()) >= event.getQuantity()) {
                    UpdateStatusProductCommand command =
                            new UpdateStatusProductCommand(event.getProductId(),
                                    event.getQuantity(), event.getUserId(),
                                    event.getId());
                    commandGateway.sendAndWait(command);
                } else {
                    kafkaTemplate.send("notificationTopic", "Sach da het");
    ...
    private void handle(ProductUpdateStatusEvent event) {
        ...
        try {
            ...
            UserResponseCommonModel employeeResponseCommonModel = queryGateway.query(getDetailsEmployeeQuery, ResponseTypes.instanceOf(UserResponseCommonModel.class)).join();
            if (employeeResponseCommonModel.getEnabled() == true) {
                kafkaTemplate.send("notificationTopic", "Khach hang bi ky luat");
                throw new Exception("Khach hang bi ky luat");
            } else {
                kafkaTemplate.send("notificationTopic", "Da them vao gio hang" +
                        " thanh cong");
                SagaLifecycle.end();
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
            rollBackBookStatus(event.getId(), event.getUserId(), event.getCartId(), event.getSoldQuantity());
        }
    }

    private void rollBackBookStatus(String id, Long user_id, String cart_id, int number) {
        SagaLifecycle.associateWith("id", id);
        RollBackStatusProductCommand command = new RollBackStatusProductCommand(id, number, user_id, cart_id);
        commandGateway.sendAndWait(command);
    ...
    public void handleRollBackBookStatus(ProductRollBackStatusEvent event) {
        ...
        rollBackShoppingCartRecord(event.getCartId());
    }
    ...
    private void rollBackShoppingCartRecord(String id) {
        try {
            ...
            System.out.println("Error occurred while rolling back shopping cart record: " + e.getMessage());
        }
    }
