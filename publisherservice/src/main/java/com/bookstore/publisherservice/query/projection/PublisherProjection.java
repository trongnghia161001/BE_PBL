...
import com.bookstore.commonservice.model.PublisherResponseCommonModel;
import com.bookstore.commonservice.query.GetProductByPublishNameQuery;
...
public class PublisherProjection {
    ...
    @QueryHandler
    public PublisherResponseCommonModel getAll(GetProductByPublisherQuery getProductByPublisherQuery) {
        PublisherResponseCommonModel publisherResponseCommonModel = new PublisherResponseCommonModel();
        Optional<Publisher> publisher = repository.findById(getProductByPublisherQuery.getPublisherId());
        if (publisher.isPresent()) {
            BeanUtils.copyProperties(publisher.get(), publisherResponseCommonModel);
            return publisherResponseCommonModel;
        } else {
            return null;
        }
    }
    ...
    @QueryHandler
    public List<PublisherResponseCommonModel> getAll(GetProductByPublishNameQuery nameQuery) {
        List<PublisherResponseCommonModel> publisherResponseCommonModel =
                new ArrayList<>();
        List<Publisher> publisher =
                repository.findByNameContainingIgnoreCase(nameQuery.getPublishName());
        if (publisher.size() > 0) {
            publisher.forEach(p -> {
                PublisherResponseCommonModel responseModel = new PublisherResponseCommonModel();
                BeanUtils.copyProperties(p, responseModel);
                publisherResponseCommonModel.add(responseModel);
            });
            return publisherResponseCommonModel;
        } else {
            return null;
