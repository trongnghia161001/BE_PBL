...
import com.bookstore.commonservice.model.ProductTypeResponseCommonModel;
...
public class ProductTypeProjection {
    ...
    public List<ProductResponseCommonModel> getAllBook(GetAllProductTypeByNameQuery nameQuery) {
        ...
        List<ProductType> productTypes = repository.findByNameContainingIgnoreCase(nameQuery.getName());
    ...
    @QueryHandler
    public ProductTypeResponseCommonModel getAll(GetProductByProductTypeQuery getProductByProductTypeQuery) {
        ProductTypeResponseCommonModel productTypeResponseCommonModel = new ProductTypeResponseCommonModel();
        Optional<ProductType> productType = repository.findById(getProductByProductTypeQuery.getProductTypeId());
        if (productType.isPresent()) {
            BeanUtils.copyProperties(productType.get(), productTypeResponseCommonModel);
            return productTypeResponseCommonModel;
        } else {
            return null;
        }
    }
