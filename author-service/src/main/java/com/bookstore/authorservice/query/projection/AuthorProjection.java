...
import com.bookstore.commonservice.query.GetProductByAuthorNameQuery;
...
public class AuthorProjection {
    ...
    @QueryHandler
    public List<AuthorResponseCommonModel> getAllProductByAuthorName(GetProductByAuthorNameQuery getProductByAuthorNameQuery) {
        List<AuthorResponseCommonModel> authorResponseCommonModels = new ArrayList<>();
        List<Author> author = repository.findByNameContainingIgnoreCase(getProductByAuthorNameQuery.getAuthorName());

        if (!author.isEmpty()) {
            author.forEach(a -> {
                AuthorResponseCommonModel authorResponseCommonModel = new AuthorResponseCommonModel();
                BeanUtils.copyProperties(a, authorResponseCommonModel);
                authorResponseCommonModels.add(authorResponseCommonModel);
            });
        }

        return authorResponseCommonModels;
    }
